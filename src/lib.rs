...
async fn run() {
        ...
        entries: &[
            wgpu::BindGroupEntry {
                ...
                resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                    buffer: &globals_ubo,
                    offset: 0,
                    size: Option::from(std::num::NonZeroU64::new(globals_buffer_byte_size))
                }),
            },
            wgpu::BindGroupEntry {
                binding: 1,
                resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                    buffer: &prims_ubo,
                    offset: 0,
                    size: Option::from(std::num::NonZeroU64::new(prim_buffer_byte_size))
                }),
            },
        ],
    });
    ...
    event_loop.run(move |event, _, control_flow| {
...

        {
            ...
            pass.set_index_buffer(ibo.slice(..), wgpu::IndexFormat::default());
            ...
            if scene.draw_background {
                ...
                pass.set_bind_group(0, &bind_group, &[]);
                pass.set_index_buffer(bg_ibo.slice(..), wgpu::IndexFormat::default());
